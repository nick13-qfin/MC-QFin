...
BOOST_AUTO_TEST_CASE(constr_singlescheme)
{
	...
	mc::mc_engine< mc::euler_scheme<gbm_t>> engine(1,1, std::move(scheme));
...
BOOST_AUTO_TEST_CASE(constr_tuple)
{
    ...
    auto mc_engine = mc::make_mcengine(1, 1,  std::move(scheme), std::move(scheme2));
...
BOOST_AUTO_TEST_CASE(calculate)
{
    ...
    auto mc_engine = mc::make_mcengine(1000000, 1, std::move(scheme));
	utils::stopwatch sw{};
	mc::dummy_payoff payoff{};
	sw.start();
	mc_engine.calculate(payoff);
	sw.stop();
	auto value = mc_engine.get_results().get_estimate();
